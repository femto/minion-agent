    ...
    async def _send_query(
        ...
        async with httpx.AsyncClient(follow_redirects=True) as query_client:
            ...
            if not response.root:
                msg = (
                    "The A2A agent did not return a root. Are you using an A2A agent not managed by minion-agent? "
                    "Please file an issue at https://github.com/mozilla-ai/minion-agent/issues so we can help."
                )
            ...
            if isinstance(response.root, JSONRPCErrorResponse):
                ...
                msg = (
                    "The A2A agent did not return a error or a result. Are you using an A2A agent not managed by minion-agent? "
                    "Please file an issue at https://github.com/mozilla-ai/minion-agent/issues so we can help."
                )
                raise ValueError(msg)
